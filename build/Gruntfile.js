...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    copy: {
      build: {
        cwd: '<%= config.src %>',
        src: [
          'virtusauidocs/assets/**/*.*',
          'virtusauidocs/partials/*.html',
          'virtusauidocs/prettify.css',
          // 'virtusauidocs/js/google-code-prettify/prettify.js',
          // 'virtusauidocs/js/virtusauidocs.js',
          // 'virtusauidocs/js/virtusauidocs-menu.js',
          // 'virtusauidocs/js/index.js',
          // 'virtusauidocs/js/load-partials.js',
          'assets/**/*.*',
          'css/base/fonts/*.*',
    ...
    archive: {
      cwd: 'virtusaui',
      src: ['../src', '../build/Gruntfile.js', '../build/package.json', '../Virtusa Pattern Library Setup.docx'],
      dest: '../virtusaui.zip'
    },
    createInlineTemplates: {
      cwd: '<%= config.src %>',
      src: ['virtusauidocs/partials', 'templates'],
    ...
    sass: {
      ...
      dist: {
        files: [{
          "<%= config.staging %>/<%= config.css %>/nylui.css": "<%= config.src %>/css/mainui.scss",
          "<%= config.staging %>/<%= config.css %>/nylexternal.css": "<%= config.src %>/css/mainexternal.scss",
          "<%= config.staging %>/<%= config.virtusauidocs %>/virtusauidocs.css": "<%= config.src %>/<%= config.virtusauidocs %>/virtusauidocs.scss"
        }]
      }
    },
    postcss: {
    ...
    watch: {
      files: ['<%= config.src %>/virtusauidocs/**/*.*', '<%= config.src %>/assets/**/*.*', '<%= config.src %>/css/**/*.*', '<%= config.src %>/js/**/*.*',
    ...
    var processTemplates = function () {
      ...
      function fixTemplateTarget(name, path) {
        if (path === 'templates/layout/foundation.html' || path === 'templates/layout/grid.html' || path === 'templates/layout/swatch.html') {
          ...
          return 'layouts';
        } else if (path === 'virtusauidocs/partials/setup.html') {
          return 'setup';
        }
        return name;
      }
      ...
      for (var index = 0; index < source.length; index++) {
        ...
        for (var f = 0; f < files.length; f++) {
          ...
          if (ext === '.html') {
            ...
            if (temp.indexOf('virtusauidocs-partials-code') !== -1 && PROCESS_SNIPPETS) {
              jsdom = require("jsdom");
               ...
               * The all snippets.
               */
              var allSnippets = $('.virtusauidocs-partials-code');
      ...
      writeFile('../staging/virtusauidocs/partial-config.js', "var loaderConfig = " + JSON.stringify(loaderConfigBuilder, null, 2) + ";");
      /**
       * The index.
       */
  ...
  grunt.registerTask('optimize', function () {
    ...
    config = {
      ...
      mainConfigFile: '../src/virtusauidocs/js/index.js',
     ...
     * @type {string}
     */
    var basePath = '../src/virtusauidocs/js/';
    /**
     * The new index.
     * 
     * @type {string}
    ...
    buffer.push("\tvirtusauidocs.setOneTimeInit(oneTimeInit);");
    buffer.push("\tvirtusauidocs.setTemplateInit(templateInit);");
    buffer.push("\tvirtusauidocs.setPartials(" + JSON.stringify(partials) + ");");
    buffer.push("});");


    
    newIndex += buffer.join('\n');
    writeFile('../staging/virtusauidocs/js/' + 'index.js', newIndex);
    done();
  });
  // load tasks
